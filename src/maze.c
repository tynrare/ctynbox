...
float distlerp(float a, float b, float t) {
  float tt = min(1.0f, 1.0f / (fabsf(b - a) + 0.01));
  return a + (b - a) * t * tt;
}
...
void _TynmazeInit(TynmazeState *state) {
  ...
  // --- textures
  state->tex_mark = LoadTexture(RES_PATH "tynmaze/flag_triangle.png");

  // --- map
  Image imMap =
      LoadImage(RES_PATH "tynmaze/maze-0.png"); // Load cubicmap image (RAM)
  state->cubicmap =
  ...
  state->texture =
      LoadTexture(RES_PATH "tynmaze/cubicmap_atlas.png"); // Load map texture
...
TynmazeState *TynmazeInit(TynStage *stage) {
  ...
  stage->state = state;
  stage->frame =
      (TynFrame){&TynmazeDispose, &TynmazeStep, &TynmazeDraw, NULL, &cmd};
...
void TynmazeStepPawn(TynmazeState *state) {
  ...
  if (IsKeyPressed(KEY_W)) {
    ...
    float x =
        state->pawn.playerPosition.x + sinf(state->pawn.playerTurn) * 0.25f;
    float y =
        state->pawn.playerPosition.y + cosf(state->pawn.playerTurn) * 0.25f;
    state->pawn.tagPositions[index] = (Vector2){x, y};
    state->pawn.tagsCount =
        min(state->pawn.tagsCount + 1, TYNMAZE_PAWN_TAGS_LIMIT);
  }
  ...
  while (!collider && state->pawn.inputDirection.x) {
    float newx =
        roundf(state->pawn.playerPosition.x +
               sinf(state->pawn.playerTurn) * state->pawn.inputDirection.x);
    float newy =
        roundf(state->pawn.playerPosition.y +
...
state->pawn.playerTurn);
  ...
  case TMZ_VIEW_MODE_PAWN_TOPDOWN: {
    ...
    state->camera.target.z =
        lerp(state->camera.target.z, state->pawn.playerPosition.y, 0.55f);
  } break;
  default:
    break;
  }
  ...
  SetShaderValue(state->shader, state->shader.locs[SHADER_LOC_VECTOR_VIEW],
                 &state->camera.position.x, SHADER_UNIFORM_VEC3);
}

void TynmazeStepFree(TynmazeState *state) {
  state->camera.position.x = state->cubicmap.width / 2.0f;
  state->camera.position.y = 40.0f;
  state->camera.position.z = state->cubicmap.width / 2.0f;
...
void TynmazeDraw(TynmazeState *state) {
  ...
  DrawModel(state->model, state->pawn.mapPosition, 1.0f,
            WHITE); // Draw maze map

  for (int i = 0; i < min(state->pawn.tagsCount, TYNMAZE_PAWN_TAGS_LIMIT);
       i++) {
    float x = state->pawn.tagPositions[i].x;
    float y = state->pawn.tagPositions[i].y;
    DrawBillboard(state->camera, state->tex_mark, (Vector3){x, 0.5f, y}, 0.25f,
                  WHITE);
