...
const char *Raymarch2dfragShaderFileName = "res/shaders/tynroar-sdf-%i.fs";

Color Raymarch2dPosToColor(int x, int y) {
  return (Color){(unsigned char)x, (unsigned char)((int)x >> 8),
 ...
 */
int Raymarch2dWriteEntity(int shift, Raymarch2dShapeType type, Vector2 position,
                          Vector2 size, int rotation) {
  DrawPixelV(Raymarch2dIndexToPos(shift, Raymarch2dDatasetSize),
             (Color){4, 0, 0, 0});

  // entity type
  // a == 0 : circle
  // a == 1 : box
  DrawPixelV(Raymarch2dIndexToPos(shift + 1, Raymarch2dDatasetSize),
             (Color){(int)type, 0, 0, 0});
  // entity position
  DrawPixelV(Raymarch2dIndexToPos(shift + 2, Raymarch2dDatasetSize),
             Raymarch2dPosToColor((int)position.x, (int)position.y));
  // entity size
  DrawPixelV(Raymarch2dIndexToPos(shift + 3, Raymarch2dDatasetSize),
             Raymarch2dPosToColor(size.x, size.y));
  // entity rotation
  DrawPixelV(Raymarch2dIndexToPos(shift + 4, Raymarch2dDatasetSize),
             Raymarch2dPosToColor(rotation, 0));
